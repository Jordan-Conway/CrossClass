...
struct Class_Info* update_class_info_with_line(struct Class_Info* class_info, struct Line_Data_Node** line_data)
{
    return class_info;
}
...
struct Data_Parser_Result* parse_line_data(struct Line_Data_Node* line_data_list){
    ...
    while(line_data_list != NULL)
    {
        update_class_info_with_line(class_info, &line_data_list);
    }
